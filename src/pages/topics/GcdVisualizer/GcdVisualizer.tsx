...
import { computeEuclideanBeats, TileFitEuclidean } from './TileFitEuclidean';
...
export function GcdVisualizer() {
  ...
  const [interStep, setInterStep] = useState(0);
  const [interMode, setInterMode] = useState<'manual' | 'auto'>('manual');
  const [euclidStep, setEuclidStep] = useState(0);
  const [euclidMode, setEuclidMode] = useState<'manual' | 'auto'>('manual');

  useEffect(() => {
    setInterStep(0);
    setInterMode('manual');
    setEuclidStep(0);
    setEuclidMode('manual');
  }, [a, b]);
  ...
  const interClamped = Math.min(interStep, interTotalSteps - 1);
  ...
  const euclidTotalSteps = Math.max(euclidBeats.beats.length, 1);
  const euclidClamped = Math.min(euclidStep, euclidTotalSteps - 1);

  // ---------- active method bindings for the shared footer ----------
  const activeTotalSteps = method === 'intersection' ? interTotalSteps : euclidTotalSteps;
  const activeStep = method === 'intersection' ? interClamped : euclidClamped;
  const activeMode = method === 'intersection' ? interMode : euclidMode;
  const setActiveStep = method === 'intersection' ? setInterStep : setEuclidStep;
  const setActiveMode = method === 'intersection' ? setInterMode : setEuclidMode;

  return (
    <div className={styles.root}>
      <div className={styles.header}>
        <h3 className={styles.appTitle}>Interactive GCD Visualizer</h3>
      ...
      <div className={styles.canvas}>
        {method === 'intersection' ? (
          ...
          <TileFitEuclidean a={a} b={b} beatIndex={euclidClamped} />
        )}
      </div>
