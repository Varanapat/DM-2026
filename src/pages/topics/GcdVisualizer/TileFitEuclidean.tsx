...
export const TILE_PALETTE = ['var(--tile-blue, #38bdf8)', 'var(--color-success)', '#a78bfa', '#f472b6'];

/** Execution Monitor content for a given beat — pure, shared with GcdVisualizer. */
export function tileFitMonitorData(
  a: number,
  b: number,
  beatIndex: number,
): { hint: string; lines: MonitorLine[]; badge: string | undefined } {
  const { beats, steps } = computeEuclideanBeats(a, b);
  const beat = beats[Math.min(beatIndex, beats.length - 1)] ?? beats[0];
  const currentRound = beat.kind === 'setup' ? -1 : beat.round;
  const isDone = currentRound === steps.length - 1;

  return {
    hint: `พื้นที่ ${Math.max(a, b)} × ${Math.min(a, b)} — กด ▶ เพื่อเริ่มปูตาราง`,
    lines: steps.slice(0, currentRound + 1).map((step, r) => ({
      swatchColor: TILE_PALETTE[r % TILE_PALETTE.length],
      text: `GCD(${step.a}, ${step.b}) ➔ ${step.a} mod ${step.b} = ${step.remainder}`,
    })),
    badge: isDone ? `GCD found: ${gcd(a, b)}!` : undefined,
  };
}
...
/** The SVG stage only — chrome and monitor live in VisualizerFrame/ExecutionMonitor. */
export function TileFitEuclidean({ a, b, beatIndex }: TileFitEuclideanProps) {
  ...
  return (
    <svg className={styles.stage} viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} role="img" aria-label="เรขาคณิตแสดงขั้นตอนยูคลิด">
      <defs>
        <pattern id={gridPatternId} patternUnits="userSpaceOnUse" x={floor.x} y={floor.y} width={scale} height={scale}>
      ...
      {rounds.slice(0, currentRound + 1).map((round, r) =>
        round.squares.map((sq, i) => (
          <rect
            ...
            style={{ '--tile-index': r === currentRound ? i : 0, fill: TILE_PALETTE[r % TILE_PALETTE.length] } as React.CSSProperties}
